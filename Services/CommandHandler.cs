...
public class CommandHandler
{
    ...
    {
        var models = new[] { "pebbles-3.5-sonnet", "pebbles-3.5-opus", "pebbles-4-haiku", "pebbles-4-sonnet" };
        ...
        if (models.Contains(target))
        {
            ...
            {
                Success = true,
                Message = $"Switched to model: {target}"
            });
        }
        ...
        {
            Success = false,
            Message = $"Unknown model: {target}. Use /model to see available models."
        });
    }
    ...
    private Task<CommandResult> HandleCompact(string[] args, ChatSession session)
    {
        ...
        {
            Success = true,
            Message = $"Compact mode: {(session.CompactMode ? "ON" : "OFF")} — thinking blocks will be {(session.CompactMode ? "hidden" : "shown")}."
        });
    }
    ...
    private Task<CommandResult> HandleHistory(string[] args, ChatSession session)
    {
        if (session.Messages.Count == 0)
            return Task.FromResult(new CommandResult { Success = true, Message = "No messages yet." });
